...
/**
 * Upload a PDF/DOCX document. Uses Cloudinary raw upload when configured,
 * otherwise saves under ./uploads and serves via /uploads (development).
 */
export async function uploadDocument(buffer: Buffer, folder: string, filename: string): Promise<UploadResult> {
  if (env.cloudinary.enabled) {
...
async function uploadDocumentToCloudinary(buffer: Buffer, folder: string, filename: string): Promise<UploadResult> {
  const timestamp = Math.floor(Date.now() / 1000);
  const publicId = `carlspat/${folder}/${path.parse(filename).name}-${timestamp}`;
  const toSign = `public_id=${publicId}&timestamp=${timestamp}${env.cloudinary.apiSecret}`;
  const signature = crypto.createHash("sha1").update(toSign).digest("hex");
  ...
  const res = await fetch(
    `https://api.cloudinary.com/v1_1/${env.cloudinary.cloudName}/raw/upload`,
    { method: "POST", body: form }
  );
  if (!res.ok) {
    throw new Error(`Cloudinary upload failed: ${res.status} ${await res.text().catch(() => "")}`);
